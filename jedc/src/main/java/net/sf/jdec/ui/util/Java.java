/*
 * Java.java Copyright (c) 2006,07 Swaroop Belur 
 *  
...
public class Java extends JDialog implements ActionListener {
	
	
	
	
	public Java( JFrame frame ) {
	...
	private void runFile() throws IOException
	{
		...
		if(txt1!=null)
		{
			...
			if(str.length() > 0)
			{
				...
				Process p=null;
				if(javaHome!=null){
					java.lang.String cmd=javaHome+File.separator+"bin"+File.separator+"java.exe ";
					File ja=new File(cmd);
					if(!ja.exists())
					{
						JOptionPane.showMessageDialog(this,"Please set java Home First[Edit-->preferences]");
						return ;
					}
					if(debug)cmd+="-g ";
				...
				{
					JOptionPane.showMessageDialog(UILauncher.getMainFrame(), "Java Home Not Set!", "Java Error", JOptionPane.ERROR_MESSAGE);
				}
			}
		}
	...
	private void setResultOutputFile(String file)
	{
		...
		if(tmp!=null && file!=null)
		{
			...
			if(lbl!=null)lbl.setFont(new Font(null,Font.BOLD,12));
		}
