		...
		for (int s = 0; s < code.length; s++) 
		{
			...
			else 
			{
				...
				if (!invoke) 
				{
					...
					if (toskip == -3) // wide // TODO ?
					{
						xx = true;
						for (int q=0;q<6;q++)
							System.out.println("code["+(s+q)+"]="+code[s+q]);
						s = s + 6;
					}
			...
			if (invoke) // && !(sb.toString().equalsIgnoreCase("special")))
			{
				...
				while (z >= 0) 
				{
					...
					//
					Integer isinvokestart = (Integer) GlobalVariableStore.getInvokeStartEnd().get(new Integer(start));
					while (isinvokestart != null) 
					{
						...
						if (refstart == null) 
						{
							GlobalVariableStore.getProblematicInvokes().add(new Integer(s));
							skipthisinvoke = true;
							skiploop = true;
							break;
						}
						...
						isinvokestart = (Integer) GlobalVariableStore.getInvokeStartEnd().get(new Integer(h));
						...
						if (l != null) 
						{
							...
							if (bracket != -1) 
							{
								...
								if (GlobalVariableStore.getVariableDimAss() != null)
									GlobalVariableStore.getVariableDimAss().put(new Integer(in), new Integer(totalDimenForVar));
							} 
							else 
							{
					...
					if (in == -200) 
					{
						GlobalVariableStore.getPushTypes().put(new Integer(ipos), type);
					}
				...
				if (!ok && prev >= 0 && !skip2 && !skipthisinvoke) 
				{
					...
					Integer be = (Integer) GlobalVariableStore.getInvokeStartEnd().get(pr);
					if (be != null) 
					{
						int Ibe = be.intValue();
				...
				if (GlobalVariableStore.getInvokeStartEnd() != null) 
				{
					...
					GlobalVariableStore.getInvokeStartEnd().put(new Integer(end), new Integer(s));
				}
