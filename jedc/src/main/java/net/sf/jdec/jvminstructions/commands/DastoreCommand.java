...
public class DastoreCommand extends AbstractInstructionCommand {

	public DastoreCommand(Behaviour context) {
		super(context);
	...
	public void execute() {
		...
		boolean bl2 = DecompilerHelper
				.checkForArrayMultiAssignablePostIncrement(currentForIndex,
						opStack, behaviour.getCode(), "category1");
		if (b && opStack.size() > 0
				...
				if (GlobalVariableStore.getArraytimesstack().size() == 0) {
					...
					if (curpos - 2 > 0
						...
						specialIASTORE = true;
							...
							&& info[curpos - 1] == JvmOpCodes.DADD
							&& info[curpos - 2] == JvmOpCodes.DCONST_1
