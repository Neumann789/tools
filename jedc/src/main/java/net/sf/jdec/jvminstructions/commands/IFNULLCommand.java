...
public class IFNULLCommand extends AbstractInstructionCommand {

	public IFNULLCommand(Behaviour context) {
		super(context);
	...
	public void execute() {
		...
		boolean processIF = true;// checkForTernaryIf(ifst, info,
									// op.getOperandValue() + " != null");
		if (processIF) {
			...
			if ((ifst.getIfCloseLineNumber() > 0 && ifst.getIfCloseLineNumber() < info.length)
				...
				if (gotoIndex < (t + 3)) {
					...
					while (infLoop.hasNext()) {
						...
						if (iloop.getStartIndex() == gotoIndex
							...
							boolean c = IFHelper.addCodeStatementWRTShortcutOR(
									ifst, op.getOperandValue() + "==null",
									print, "if", last, op.getOperandValue()
											+ "!=null");
							if (c) {
								if (addifbreak) {
					...
					boolean last = IFHelper.lastIFinShortCutChain(info, ifst,
							currentForIndex);
					c = IFHelper.addCodeStatementWRTShortcutOR(ifst, op
							.getOperandValue()
