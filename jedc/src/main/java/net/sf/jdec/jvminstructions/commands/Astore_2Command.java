...
public class Astore_2Command extends AbstractInstructionCommand {

	public Astore_2Command(Behaviour context) {
		super(context);
	...
	public void execute() {
		...
		if (add == true && !doNotPop) {
			...
			LocalVariable local = DecompilerHelper.getLocalVariable(2, "store",
				...
				if (!push) {
					...
					if (!local.isDeclarationGenerated()) {
						...
						if (!multinewfound) {
							...
							String typeToCheck  = local.getDataType()
							.replace('/', '.');
						
							DecompilerHelper.checkForImport(typeToCheck, m);
