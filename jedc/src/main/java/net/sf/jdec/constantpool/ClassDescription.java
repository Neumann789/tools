...
public class ClassDescription {
	...
	public void readClassAndSuperClassInfo(DataInputStream dis)
			throws IOException {
	...
	public void readInterfaceInformation(DataInputStream dis)
			throws IOException {
		...
		for (int index = 0; index < interfaceCount; index++) {
			...
			interfacesImplemented.add(getUTF8String(cinfo.getUtf8pointer())
					.replace('/', '.'));
		}
	}
	...
	public void readFieldInformation(DataInputStream dis) throws IOException {
		...
		for (int index = 0; index < fieldCount; index++) {
			...
			if (objectType.startsWith(Constants.ISREFERENCE)
					&& desc.endsWith(";")) {
				java.lang.String class_type = desc.substring(0,
						desc.length() - 1);
			...
			if (desc.indexOf(Constants.ISARRAY) != -1) {
				...
				if (objectType.startsWith(Constants.ISREFERENCE)
						&& desc.endsWith(";")) {
					java.lang.String class_type = objectType.substring(0,
							objectType.length() - 1);
					// finfo.addFieldType("L");
					finfo.addFieldType(class_type);
			...
			for (int k = 0; k < attributes_count; k++) {
				...
				if (getUTF8String(attribute_name_index).equalsIgnoreCase(
						"constantvalue")) {
					int attribute_length = dis.readInt();
					int val_index = dis.readUnsignedShort();
					if (!finfo.isArray()
							&& (finfo.getFieldType().get(0).equals(
									Constants.ISINT) || finfo.getFieldType()
									.get(0).equals(Constants.ISSHORT))
							|| finfo.getFieldType().get(0).equals(
									Constants.ISBYTE)
							|| finfo.getFieldType().get(0).equals(
									Constants.ISCHAR)) {
						finfo.setObj(new Integer(
								getINTPrimitiveAtCPoolPosition(val_index)
										.getValue()));
					}
					if (!finfo.isArray()
							&& finfo.getFieldType().get(0).equals(
									Constants.ISFLOAT)) {
						float value = getFloatPrimitiveAtCPoolPosition(
								val_index).getValue();
					...
					if (!finfo.isArray()
							&& finfo.getFieldType().get(0).equals(
									Constants.ISDOUBLE)) {
						double value = getDoublePrimitiveAtCPoolPosition(
								val_index).getValue();
					...
					if (!finfo.isArray()
							&& finfo.getFieldType().get(0).equals(
									Constants.ISLONG)) {
						long value = getLongPrimitiveAtCPoolPosition(val_index)
								.getValue();
						finfo.setObj(new Long(value));
					}
					if (!finfo.isArray()
							&& ((java.lang.String) finfo.getFieldType().get(0))
									.indexOf("Ljava/lang/String") != -1) {
				...
				} else if (getUTF8String(attribute_name_index)
					...
					finfo.setSignature(sign);
						...
						.equalsIgnoreCase("RuntimeVisibleAnnotations")) {
					dis.readInt();
					...
					for (int ctr = 0; ctr < number; ctr++) {
						...
						.equalsIgnoreCase("RuntimeInvisibleAnnotations")) {
					dis.readInt();
					int number = dis.readUnsignedShort();
	...
	public void readMethodInformation(DataInputStream dis) throws IOException {
		...
		for (int i = 0; i < methodCount; i++) 
		{
			...
			System.out.println("AS: 0x" + Integer.toHexString(accessSpecifiers) + " /NI: 0x" + Integer.toHexString(nameandtypeIndex) + 
			                    " /DI: 0x" + Integer.toHexString(descriptorIndex) + " /AC: 0x" + Integer.toHexString(attributesCount));

			java.lang.String methodName = getUTF8String(nameandtypeIndex);

			java.lang.String methodDesc = getUTF8String(descriptorIndex); // NOTE:
			// used
			// only
			// for
			// passing
			...
			System.out.println("descriptor["+descriptorIndex+"] : ["+ descriptor + "]");
			...
			for (int j = 0; j < attributesCount; j++) 
			{
				...
				if (attrType.equals("Code")) 
				{
					...
					System.out.println("exc_table_len: "+exc_table_len);
					if (exc_table_len > 0)
						minfo.setExceptionTablePresent(true);
					...
					for (int indx = 0; indx < exc_table_len; indx++) 
					{
						...
						MethodInfo.ExceptionTable etable = minfo.new ExceptionTable(
								startPc, endPc, handlerPc, catchType);
						if (catchType != 0) {
							ClassInfo cinfo = this
									.getClassInfoAtCPoolPosition(catchType);
							int utf8p = cinfo.getUtf8pointer();
							java.lang.String className = this
									.getUTF8String(utf8p);
					...
					allexceptiontables = handleTablesForSynchronizedCode(
							allexceptiontables, synchronizedTables, info, minfo);
					...
					while (alltables.hasNext()) {
						...
						ExceptionTable table = (ExceptionTable) alltables
								.next();
						if (table.getExceptionName().equals("<any>") == false) {
							table.setTypeOfGuardRegion("try");
						} else {
							// Check for catch or Try
							java.lang.String guardType = checkForCatchOrTryGuard(
									table, allexceptiontables);
							if (guardType.length() == 0)
								guardType = "try";
					...
					System.out.println("attr_cnt: "+attr_cnt);
					...
					for (int counter = 0; counter < src.length; counter++) {
						...
						if (counter >= src.length)
							break;
						inst = info[counter];
						if (isThisInstrStart(startsOF, counter)
								&& inst == JvmOpCodes.LOOKUPSWITCH) {
							...
							// |info[++counter];
							counter = counter + 4;
							int numberOfLabels = getSwitchOffset(info, counter,
									"");// (info[++counter] << 24) |
							// (info[++counter] << 16) |
							// (info[++counter] << 8)
							// |info[++counter];
							...
							for (int start = 0; start < numberOfLabels; start++) {
								...
								int label = getSwitchOffset(info, counter,
										"label");
								counter = counter + 4;
								/*
								 * i1=info[++counter] ;
								...
								int offsetVal = getSwitchOffset(info, counter,
										"");// (i1 << 24) + (i2 << 16) + (i3 <<
								// 8) +(i4<<0);
								counter = counter + 4;
							...
							for (int start = 0; start < numberOfLabels; start++) {
								...
								offsetValues[start] = offsetValues[start]
										+ lookupSwitchPos;
							}
							...
							for (int starts = 0; starts < numberOfLabels; starts++) {
								...
								if (pos != -1) {
									int[] values = getCaseEnd(pos, curOffset,
											offsetValues, labels, Default,
											info, minfo, switchblk);
									Switch.Case caseObj = switchblk.new Case(
											values[0], values[2], values[1]);
									switchblk.addCase(caseObj);
								} else {
									Writer writer = Writer.getWriter("log");
									writer
											.writeLog("Invalid position(-1) found for some case in switch block\n");
									writer.writeLog("Case Label: " + curlabel);
									writer.writeLog("Method "
											+ minfo.getMethodName());
									writer.flush();
								}
							}
						...
						} else if (isThisInstrStart(startsOF, counter)
								&& inst == JvmOpCodes.TABLESWITCH) {
							int temp1 = -1;
							int temp2 = -1;
							int temp3 = -1;
							int temp4 = -1;
							...
							Switch switchblk = new Switch(tableSwitchPos,
									Default);
							mapSwitchStartsWithLabels(offsetValues, labels);
						...
						if (isThisInstrStart(startsOF, counter) && !isif
								&& inst != JvmOpCodes.GOTO
								&& inst != JvmOpCodes.GOTO_W
								&& inst != JvmOpCodes.WIDE
								&& inst != JvmOpCodes.LOOKUPSWITCH
								&& inst != JvmOpCodes.TABLESWITCH) {
						...
						if (isThisInstrStart(startsOF, counter)
							...
							if (counter - 2 >= indexInst) {
								...
								boolean isLoad = IsGotoIndexInstructionIsALoad(
										indexInst, info); // modified by
								// belurs
								isStore = doesStoreFollowLoad(indexInst, info,
										minfo);
								if (!isLoad) {
									isLoad = isGotoIndexInstructionAInvoke(
											indexInst, info);
								}
								...
								int loopEnd = -1;
								if (isif == false)
									loopEnd = getMaxGotoStartGivenJump(indexInst);
								// int
								...
								if (end > loop.getEndIndex() && end != -1) {
									if (info[loop.getEndIndex()] != JvmOpCodes.GOTO)
										loop.setLoopEndForBracket(end);
								}
								boolean isdowhile = markLoopAsDoWhileType(loop
										.getStartIndex(), info, minfo);
								if (isdowhile) {
						...
						if (isThisInstrStart(startsOF, counter)
							...
							if (isThisInstrStart(minfo.getInststartpos(),
									nextPos)
									&& src[nextPos] == JvmOpCodes.IINC) {
					...
					ArrayList uniqueLoops = Loop.removeDuplicates(minfo
							.getBehaviourLoops());
					...
					ArrayList completeList = minfo
							.getExceptionTablesSortedByStartHandlerPC();
					Iterator it = completeList.iterator();
					while (it.hasNext()) {
						// Handle Each Sublist
						ArrayList subList = (ArrayList) it.next();
						ExceptionTable etable = minfo
								.getExceptionTableForSmallestStartOfGuard(subList);
						java.lang.String typeofhandler = etable
								.getTypeOfHandlerForGuardRegion();

						if (typeofhandler.equals("CatchBlock")) {
							int endOfTry = findEndOfTry(subList, minfo);
							if (endOfTry != -1)
								etable.setEndOfGuardRegion(endOfTry);
							ExceptionTable tab = minfo
									.getExceptionTableForSmallestStartOfGuard(subList);
							...
							int TryEnd = findEndOfTryForCreatedTable(subList,
									minfo);
							if (TryEnd != -1) {
								int HandlerPC = etable
										.getStartOfHandlerForGuardRegion();
								java.lang.String exName = etable
										.getExceptionName();
								MethodInfo.ExceptionTable newTable = minfo.new ExceptionTable(
										etable.getStartPC(), etable.getEndPC(),
							...
							int startOfHandler = ((ExceptionTable) subList
									.get(0)).getStartOfHandlerForGuardRegion();
							Iterator allextables = allexceptiontables
									.iterator();
							boolean catchPresent = false;
							int catchIndex = -1;
							ArrayList catchHandlers = new ArrayList();
							while (allextables.hasNext()) {
								ExceptionTable etab = (ExceptionTable) allextables
										.next();
								if (etab.getStartOfGuardRegion() == etable
										.getStartOfGuardRegion()
										&& etab.getEndOfGuardRegion() == etable
												.getEndOfGuardRegion()
										&& etab != etable) {
							...
							if (catchPresent == true) {
								...
								for (int s = 0; s < catchHandlers.size(); s++) {
									int in = ((Integer) catchHandlers.get(s))
											.intValue();
									temp[s] = in;
								}
								Arrays.sort(temp);
							...
							if (catchPresent == true && catchIndex != -1) {
								ArrayList sortedList = minfo
										.getSortedExceptionTablesForEndOfGuard(subList);
								for (int c = (sortedList.size() - 1); c >= 0; c--) {
									ExceptionTable et = (ExceptionTable) sortedList
											.get(c);
									if (et.getEndOfGuardRegion() < catchIndex) {
										etable.setEndOfGuardRegion(et
												.getEndOfGuardRegion());
										// System.out.println();
										break;
									}
								...
								for (int c = (sortedList.size() - 1); c >= 0; c--) {
									...
									{
										etable
												.setEndOfGuardRegion(startOfHandler);
										break;
									}
					...
					if (createdList != null) {
						for (int s = 0; s < createdList.size(); s++) {
							ExceptionTable t1 = (ExceptionTable) createdList
									.get(s);
							ArrayList Alltables = minfo.getAllExceptionTables();
							for (int s2 = 0; s2 < Alltables.size(); s2++) {
								ExceptionTable et = (ExceptionTable) Alltables
										.get(s2);
								if (et.getStartPC() == t1.getStartPC()
										&& et.getEndPC() == t1.getEndPC()
										&& et.getStartOfHandler() == t1
												.getStartOfHandler()
										&& et.getExceptionName().equals(
												t1.getExceptionName())) {
									et.setEndOfGuardRegion(t1
											.getEndOfGuardRegion());
									break;
								}
							}
					...
					if (tablesWithFinally != null) {
						...
						while (it1.hasNext()) {
							...
							if (skip == false) {
								ArrayList tryTablesWithSamePCS = minfo
										.getTryTablesWithPCRange(et1
												.getStartPC(), et1.getEndPC());
								if (tryTablesWithSamePCS != null) {
									ArrayList sorted = minfo
											.getSortedExceptionTablesForEndOfGuard(tryTablesWithSamePCS);
									ExceptionTable biggest = (ExceptionTable) sorted
											.get(sorted.size() - 1);
									int biggestEnfOfGuard = biggest
											.getEndOfGuardRegion();
									if (biggestEnfOfGuard < et1
											.getStartOfHandlerForGuardRegion()) {
										et1
												.setEndOfGuardRegion(biggestEnfOfGuard);
									}
								}
							}
					...
					while (alltables.hasNext()) {
						ExceptionTable curtable = (ExceptionTable) alltables
								.next();
					...
					if (allswicthes != null) {
						...
						while (allswIterator.hasNext()) {
							...
							switchBlock.setEndOfDefault(switchBlock
									.getDefaultStart()); // Done keeping the
							// above mentioned
							// points in mind .
							// So For This
							// switch NO default
							// will be displayed
							if (skip == false) {
								int defaultStart = switchBlock
										.getDefaultStart();
								findEndOfDefaultForSwitch(switchBlock,
										defaultStart, minfo.getCode(), minfo);
							}

							setFallThruForCases(switchBlock, minfo.getCode());
							if (switchBlock.getDefaultEnd() == switchBlock
									.getDefaultStart()) {
					...
					for (int jj = 0; jj < alltries.size(); jj++) {
						...
						if (!TRY.hasFinallyBlk()) {
							...
							if (lastcatch != null) {
								...
								int newend = checkEndOFLastCatchWRTContainedLoops(
										lastcatch, loops);
								if (newend != -1) {
									if ((newend - 3) > 0)
										lastcatch.setEnd(newend - 3);
								}
								newend = checkEndOfLastCatchWRTCaseBlks(
										lastcatch, allswicthes);
								if (newend != -1) {
									if ((newend - 3) > 0)
										lastcatch.setEnd(newend - 3);
								}
								...
								for (int x = 0; x < exptables.size(); x++) {
									ExceptionTable extab = (ExceptionTable) exptables
											.get(x);
									java.lang.String type = extab
											.getTypeOfHandlerForGuardRegion();
									if (type.equalsIgnoreCase("CatchBlock")
											|| type.indexOf("Catch") >= 0
											|| type.indexOf("catch") >= 0) {
										int handlers = extab
												.getStartOfHandlerForGuardRegion();
										int handlere = extab
										...
										if (origs == handlers
												&& orige == handlere) {
											extab
													.setEndOfHandlerForGuardRegion(lastcatch
															.getEnd());
										}
					...
					for (int k = 0; k < no_of_exceptions; k++) {
						...
						minfo.addExceptions(getUTF8String(cinfo
								.getUtf8pointer()));
...

				{
					...
					Writer writer = Writer.getWriter("log");
					writer
							.writeLog("Unknown attribute encountered while parsing the attributes of a method ..."
									+ attrType);
					writer
			...
			if (!compiledWithMinusG) {
				...
				localStructure.setMethodDescription(methodName.concat(minfo
						.getStringifiedParameters()));
				LocalVariableTable localVarTable = LocalVariableTable
						.getInstance();
				localVarTable.addEntry(methodName.concat(minfo
						.getStringifiedParameters().concat(
								"" + minfo.isConstructor())), localStructure);
			...
			// minfo.getReturnType();
			// System.out.println();
			ShortcutAnalyser analyser = new ShortcutAnalyser(minfo, minfo
					.getCode());
			analyser.analysze();
	...
	private void readCodeAttributes(int attr_cnt, DataInputStream dis,
			java.lang.String methodName, MethodInfo minfo) throws IOException
		...
		for (int i = 0; i < attr_cnt; i++) 
		{
			...
			if (attrType.equals("LocalVariableTable")) 
			{
				...
				for (int count = 0; count < localVarTableLen; count++) {
					...
					if (index < minfo.getParametersArrayLength()) {
						LocalVariable localvar = new LocalVariable(methodName
								.concat(minfo.getStringifiedParameters()),
								localVardescription, localvarName, index, true);
						...
						LocalVariable localvar = new LocalVariable(methodName
								.concat(minfo.getStringifiedParameters()),
								localVardescription, localvarName, index);
						localvar.setBlockStart(start_pc);
						localvar.setBlockEnd(start_pc + len);
						variableList.add(localvar);
					}
				...
				LocalVariableStructure localStructure = new LocalVariableStructure();
				localStructure.setMethodDescription(methodName.concat(
						minfo.getStringifiedParameters()).concat(
						"" + minfo.isConstructor()));
				...
				LocalVariableTable localVarTable = LocalVariableTable
						.getInstance();
				localVarTable.addEntry(methodName.concat(
						minfo.getStringifiedParameters()).concat(
						"" + minfo.isConstructor()), localStructure);
			...
			if (attrType.equals("LineNumberTable")) 
			{
				...
				for (int count = 0; count < lineNumTableLen; count++) 
				{
					...
					System.out.println("start_pc : " + start_pc + " line_num : " + line_num);
			...
			if (attrType.equals("StackMapTable")) 
			{
				int attr_len = dis.readInt(); // Attribute Length
			...
			if (attrType.equals("LocalVariableTypeTable")) 
			{
				...
				for(int z=0;z<elementCount;z++){
					...
					if(!sign.startsWith("L"+className))
							typeTable.elements.add(element);
				}
			}
	...
	public void populateReflection(Hashtable table) {
		...
		for (int kk = 0; kk < allFields.length; kk++) {
			...
			field.setRuntimeVisibleAnnotations(finfo
					.getRuntimeVisibleAnnotations());
			field.setRuntimeInvisibleAnnotations(finfo
					.getRuntimeInvisibleAnnotations());
			if (!finfo.isArray()) {
				field.setDataType((java.lang.String) finfo.getFieldType()
						.get(0));
		...
		for (int kk = 0; kk < allMethods.length; kk++) {
			...
			if (minfo != null) {
				if (minfo.isConstructor()) {
					...
					cmember.setRuntimeVisibleAnnotations(minfo
							.getRuntimeVisibleAnnotations());
					cmember.setRuntimeInvisibleAnnotations(minfo
							.getRuntimeInvisibleAnnotations());
					cmember.setRuntimeVisibleParameterAnnotations(minfo
							.getRuntimeVisibleParameterAnnotations());
					cmember.setRuntimeInvisibleParameterAnnotations(minfo
					...
					cmember
							.setExceptionTableList(minfo
									.getAllExceptionTables());
					cmember.setAllTriesForMethod(minfo
							.getAllTryBlocksForMethod());
					cmember.setCreatedTableList(minfo.getCreatedTableList());
					cmember.setAllswitches(minfo.getAllSwitchBlksForMethod());
					cmember.setSynchronizedTableEntries(minfo
					...
					mmember.setRuntimeVisibleAnnotations(minfo
							.getRuntimeVisibleAnnotations());
					mmember.setRuntimeInvisibleAnnotations(minfo
							.getRuntimeInvisibleAnnotations());
					mmember.setRuntimeInvisibleParameterAnnotations(minfo
							.getRuntimeInvisibleParameterAnnotations());
					mmember.setRuntimeVisibleParameterAnnotations(minfo
							.getRuntimeVisibleParameterAnnotations());
					...
					java.lang.String[] methodSpecifiers = minfo
							.getAccessSpecifiers();
	...
	private java.lang.String checkForCatchOrTryGuard(ExceptionTable table,
			ArrayList allexceptiontables) {
		...
		for (int counter = 0; counter < allexceptiontables.size(); counter++) {
			...
			ExceptionTable currentTable = (ExceptionTable) allexceptiontables
					.get(counter);
	...
	private Hashtable getAllReturnsFromoffset(int jsrJumpOffet, byte[] code,
			MethodInfo minfo) {
	...
	private int findEndOfTry(ArrayList subList, MethodInfo minfo) {
		int endoftry = -1;
		if (subList.size() > 0) {
			int handlerPc = ((ExceptionTable) subList.get(0))
					.getStartOfHandlerForGuardRegion();
			int prevendOfGuard = ((ExceptionTable) subList.get(0))
					.getEndOfGuardRegion();
			for (int s = 1; s < subList.size(); s++) {
				if (subList.size() > 1) {
					int curendOfGuard = ((ExceptionTable) subList.get(s))
							.getEndOfGuardRegion();
					if (curendOfGuard > prevendOfGuard
							&& curendOfGuard <= handlerPc) {
		...
		if (endoftry != -1) {
			ExceptionTable Table = minfo
					.getExceptionTableWithBiggestStartOfGuard(subList);
			int biggestStart = -1;
			int startOfGuard = -1;
			int endOfGuard = -1;
			if (Table != null) {
				...
				// Get ExceptionTable
				ExceptionTable firstTab = minfo
						.getExceptionTableForSmallestStartOfGuard(subList);
				startOfGuard = firstTab.getStartOfGuardRegion();
				endOfGuard = firstTab.getEndOfGuardRegion();
				ArrayList sameGuards = minfo.getTableListWithGuardsSpecified(
						startOfGuard, endOfGuard);
				if (sameGuards != null) {
	...
	private void resetEndOfTries(ArrayList subList, ExceptionTable tab,
			MethodInfo minfo) {
		...
		if (temp.size() > 0) {
			...
			while (iterator.hasNext()) {
				...
				for (int s = 0; s < alltables.size(); s++) {
					...
					if (extab.getStartOfGuardRegion() == t
							.getStartOfGuardRegion()
							&& extab.getEndOfGuardRegion() == t
									.getEndOfGuardRegion()) {
		...
		for (int s = 0; s < allTables.size(); s++) {
			ExceptionTable extab = (ExceptionTable) allTables.get(s);
			if (extab.getStartPC() == tab.getStartPC()
					&& extab.getEndPC() == tab.getEndPC()) {
	...
	private java.lang.String pollForIfOrSwitch(int whereTostart,
			byte[] codeSrc, int inst, MethodInfo minfo) {
		...
		while (!endCheck) {
			...
			if (str.length() > 0)
				break; // Just a check to ensure we break out of while loop
			...
			if (start >= codeSrc.length)
				break; // Just a check to avoid exception
	...
	private int getSwitchStart(int whereTostart, byte[] codeSrc,
			MethodInfo minfo) {
		int switchpos = -1;
		...
		while (start < codeSrc.length) {
			if (isThisInstrStart(minfo.getInststartpos(), start)) {
				if (currentInstruction == JvmOpCodes.LOOKUPSWITCH
						|| currentInstruction == JvmOpCodes.TABLESWITCH)// Handle
				// tableswitch
				// also
				// here
				{
		...
		for (int i = 0; i < allTries.size(); i++) {
			...
			if (i == 0)
				...
				while (t.hasNext()) {
					...
					if (tryBlk.getStart() == Try.getStart()
							&& tryBlk.getEnd() == Try.getEnd()) {
						tryBlk.setBiggestTableUsedToCreateTry(Try
								.getTableUsedToCreateTry());
						add = false;
						break;
					} else {
						add = true;
	...
	private ArrayList checkForInvalidEntry(ArrayList subList) {
		...
		for (int i = 0; i < subList.size(); i++) {
			...
			if (et.getStartOfGuardRegion() == et
					.getStartOfHandlerForGuardRegion()) {
	...
	private void linkAllExceptionTables(MethodInfo minfo) {
		 ...
		 */
		if (list != null)
			resetEndOfGaurdValues(list, minfo);
		...
		if (list != null) {
			...
			while (iterator.hasNext()) {
				...
				ArrayList sortedTableList = minfo
						.sortTableListByStartOfHanlder(tableList);
				...
				if (hasFinally) {
					...
					for (s = 0; s < (sortedTableList.size() - 1); s++) {
						ExceptionTable et = (ExceptionTable) sortedTableList
								.get(s);
						int startOfHandler = et
								.getStartOfHandlerForGuardRegion();
						...
						if (temp != null) {
							...
							Catch.setMyStart(et
									.getStartOfHandlerForGuardRegion());// temp.getStartOfGuardRegion());
							ExceptionTable next = (ExceptionTable) sortedTableList
									.get(s + 1);
							...
							if (temp.getEndOfGuardRegion() > temp
									.getStartOfHandlerForGuardRegion()) {
							...
							ExceptionTable tb = getTableWhoseHandlerIsFinallyForThisTry(
									start, minfo
											.getAllTablesWhoseGuardsAreTries());
							if (tb != null) {
								int endg = tb.getEndOfGuardRegion();
								if (endg != -1 && endg > Catch.getStart()
										&& endg < Catch.getEnd()) {
									Catch.setEnd(endg);
								}
							}
						...
						if (et.getEndOfHandlerForGuardRegion() == -1)
					...
					for (s = 0; s < (sortedTableList.size() - 1); s++) {
						...
						newTry.addCatchBlk(Catch);
						newTry.setHasCatchBlk(true);
						et.setEndOfHandlerForGuardRegion(next
								.getStartOfHandlerForGuardRegion());
					...
					if (gotoPresent == false) {
						...
						while (sortedTableListIT.hasNext()) {
							ExceptionTable table = (ExceptionTable) sortedTableListIT
									.next();
							if (table != et) {
								int catchStart = table
										.getStartOfHandlerForGuardRegion();
					...
					if (gotoPresent == true) {
						...
						if (gotoOffset >= codeSrc.length)
							...
							CatchBlock Catch = new CatchBlock();
							Catch.setAssociatedTry(newTry);
							boolean reset = verifyEndOfCatch(newTry, Catch,
									gotoOffset, CatchStart - 3, codeSrc, minfo);
							if (!reset)
								Catch.setEnd(gotoOffset);
							Catch.setMyStart(et
									.getStartOfHandlerForGuardRegion());
							newTry.addCatchBlk(Catch);
							newTry.setHasCatchBlk(true);
							...
							if (Catch.getEnd() < Catch.getStart()) {
								...
								while (st < c.length) {
									...
									if (isThisInstrStart(minfo
											.getInststartpos(), st)
											&& c[st] == JvmOpCodes.GOTO
											&& getJumpAddress(c, st) == Catch
													.getEnd()) {
										// newpos=st+3;
										Catch.setEnd(st);
										et.setEndOfHandlerForGuardRegion(Catch
												.getEnd());
										break;
									}
									boolean retpr = false;
					...
					if (gotoPresent == false) {
						...
						while (!endCheck) {
							...
							java.lang.String IF_Switch = pollForIfOrSwitch(
									whereTostart, codeSrc, jvmInst, minfo);
							int ifoffset = -1;
							int switchDefault = -1;
							int switchStart = -1;
							...
							// switch
							{
								switchStart = getSwitchStart(whereTostart,
										codeSrc, minfo);
								if (switchStart != -1) {
									switchDefault = geSwitchDefault(
											switchStart, codeSrc);
									if (switchDefault >= 0
											&& switchDefault < codeSrc.length) {
						...
						if (ok == true) {
							...
							newCatchBlk.setMyStart(et
									.getStartOfHandlerForGuardRegion());
							newTry.addCatchBlk(newCatchBlk);
							newTry.setHasCatchBlk(true);
							handledInIfOrSwitch = true;
							newCatchBlk
									.useMeForDeterminingTheEndOfLastCatch(true);
						}
								...
								while (currentInstruction != instruction) {
									...
									if (Start < codeSrc.length) {
										...
										if (currentInstruction == instruction) {
											...
											newCatchBlk
													.useMeForDeterminingTheEndOfLastCatch(true);
											int newcatchend = checkToResetCatchEndWRTNextTry(
													minfo,
													Start,
													list,
													newTry.getStart(),
						...
						if (!done && !handledInIfOrSwitch) {
							...
							for (; i < codeSrc.length; i++) {
								if (codeSrc[i] == JvmOpCodes.ATHROW
	...
	private int checkToResetCatchEndWRTNextTry(MethodInfo minfo, int catchend,
			ArrayList list, int trystart, int catchstart) {
	...
	private void linkCreatedExceptionTables(MethodInfo minfo) {
		...
		if (list != null) {
			...
			while (iterator.hasNext()) {
				...
				Try.setStart(start);
				Try.setEnd(end);
				ArrayList tableList = minfo.getCreatedTablesWithGuardRange(
						start, end);
				int numberOfCatches;
				if (tableList != null)
					numberOfCatches = minfo.getNumberOfCatchesForTry(tableList);
	...
	private boolean SkipThisFinallyHandler(ExceptionTable et1,
			ArrayList overList) {
		boolean skip = false;
		if (overList == null || overList.size() == 0) {
			...
			for (int s = 0; s < overList.size(); s++) {
				...
				if (tab.getStartOfHandlerForGuardRegion() == et1
						.getStartOfHandlerForGuardRegion()) {
					skip = true;
					break;
				} else {
	...
	private void findEndOfDefaultForSwitch(Switch switchBlk, int defaultStart,
			byte[] code, MethodInfo minfo) {
		...
		forloop: for (; s < len; s++) {
			...
			// or Finally
			else if (minfo.getAllTryBlocksForMethod() != null
				...
				if (table != curTable) {
					...
					if (curtablestartPC == startPC) // Only if this is true
					{
						...
						// Reset
						{
							...
							if (!skip)
								curTable.setEndOfGuardRegion(table
										.getEndOfGuardRegion());
						}
	...
	private int ReturnInst(MethodInfo minfo) {
		...
		if (retType.equals("int") || retType.equals("short")
				|| retType.equals("byte") || retType.equals("char")
	...
	private boolean isInstructionASwitch(int targetJvmInst) {
		...
		if (targetJvmInst == JvmOpCodes.LOOKUPSWITCH
				|| targetJvmInst == JvmOpCodes.TABLESWITCH)
			return true;
		else
			return false;
	...
	private void findEndOfinallyForTable(ExceptionTable curtable, byte[] info,
			MethodInfo minfo) {
		if (curtable.getTypeOfHandlerForGuardRegion().equals("FinallyBlock")) {
			...
			// Handle
			{
				...
				if (jvmInstruction == JvmOpCodes.ASTORE_0
						|| jvmInstruction == JvmOpCodes.ASTORE_1
						|| jvmInstruction == JvmOpCodes.ASTORE_2
						|| jvmInstruction == JvmOpCodes.ASTORE_3) {
					isJsrPresent = checkForJSR(info, startOfFinally + 1);
					if (isJsrPresent == true)
						jsrJumpOffet = getJSRJumpOffset(info,
								startOfFinally + 1);
				...
				if (isJsrPresent == false) {
					...
					if (jvmInstruction == JvmOpCodes.ASTORE_0)
						aloadInstrcution = JvmOpCodes.ALOAD_0;
					if (jvmInstruction == JvmOpCodes.ASTORE_1)
						aloadInstrcution = JvmOpCodes.ALOAD_1;
					if (jvmInstruction == JvmOpCodes.ASTORE_2)
						aloadInstrcution = JvmOpCodes.ALOAD_2;
					if (jvmInstruction == JvmOpCodes.ASTORE_3)
						aloadInstrcution = JvmOpCodes.ALOAD_3;
						...
						if (start < info.length - 1) {
							if (simpleastore == true) {
								...
								do {
									temp = info[tempInt];
									if (isThisInstrStart(minfo
											.getInststartpos(), tempInt)
											&& temp == JvmOpCodes.ATHROW) {
										curtable
												.setEndOfHandlerForGuardRegion(tempInt);
										done = true;
										break;
									}
									boolean retpres = checkForReturn(info,
											tempInt, minfo);
									if (retpres) {
										curtable
												.setEndOfHandlerForGuardRegion(tempInt);
					...
					if (z == -1) {
						...
						if (retType.equals("int") || retType.equals("byte")
								|| retType.equals("short")
								|| retType.equals("char")
								|| retType.equals("boolean")) {
					...
					if (jvmCode == JvmOpCodes.ASTORE)
						localVarIndex = info[jsrJumpOffet + 1];
					if (jvmCode == JvmOpCodes.ASTORE_0)
						localVarIndex = 0;
					if (jvmCode == JvmOpCodes.ASTORE_1)
						localVarIndex = 1;
					if (jvmCode == JvmOpCodes.ASTORE_2)
						localVarIndex = 2;
					if (jvmCode == JvmOpCodes.ASTORE_3)
						localVarIndex = 3;
					...
					Hashtable rets = getAllReturnsFromoffset(jsrJumpOffet,
							info, minfo);
					retPos = getRetPos(rets, localVarIndex);

					// }
					if (retPresent == true && retPos != null) {
						curtable.setEndOfHandlerForGuardRegion(retPos
								.intValue());
						...
						targetJvmInst = this.getInstruction(info,
								startReadingFrom);
						...
						for (;;) {
							...
							// an if
							{
								int ifJump = getJumpAddressForIf(
										startReadingFrom, info);
								...
								iffound = true;
									...
									&& isInstructionASwitch(targetJvmInst)) // Look
							// for
							// a
							// switch
							// here
							...
							} else if (!switcchfound && !iffound
									&& targetJvmInst == JvmOpCodes.GOTO) // Hanlde
							// Goto
							// //
							// Not
							// sure
							...
							// required
							{
								int gotoJump = getGotoAddress(startReadingFrom,
										info, "normal");
							...
							if (!switcchfound
								...
								while (triesIT.hasNext()) {
									...
									if (tryblk != null) {
										...
										if (tryStart == startReadingFrom
												|| (startReadingFrom > tryStart && startReadingFrom <= tryblk
														.getEnd())) {
											tryblkfound = true;
											if (tryblk.hasFinallyBlk() == true) {
												FinallyBlock finblk = tryblk
														.getFinallyBlock();
												...
												{ // Note such problems can be
													...
													if (finEnd != -1) {
														 ...
														 */
														MethodInfo.ExceptionTable dummyExceptiontable = minfo.new ExceptionTable(
																-1, -1, -1, -1);
														dummyExceptiontable
																.setExceptionName("<any>");
														dummyExceptiontable
																.setEndOfHandlerForGuardRegion(-1);
														dummyExceptiontable
																.setStartOfHandlerForGuardRegion(finblk
																		.getStart());
														this
																.findEndOfinallyForTable(
																		dummyExceptiontable,
																		info,
																		minfo);
														int endOfFinally = dummyExceptiontable
																.getEndOfHandlerForGuardRegion();
														finblk
																.setEnd(endOfFinally);
							...
							if (!switcchfound && !iffound && !gotofound
									&& !tryblkfound
									&& (targetJvmInst == ReturnInst(minfo))) {
								endOffinallyFound = true;
								FinallyEnd = startReadingFrom;
								// hasBeenIncremented=true;
								curtable
										.setEndOfHandlerForGuardRegion(FinallyEnd);
								// System.out.println();
								break;
	...
	private boolean skipCurrentSwitch(Switch switchBlock, MethodInfo minfo) {
		...
		for (int s = 0; s < allCases.size(); s++) {
			...
			int temp3 = caseEnd - 1;
			int inst = -1;
			if (temp1 >= 0 && temp1 < code.length
					&& isThisInstrStart(minfo.getInststartpos(), temp1)) {
				inst = code[temp1];
			}
	...
	private void updateEndOfFinallyForFinallyBlocks(ExceptionTable curtable,
			ArrayList tries) {
			...
			if (tryblk.hasFinallyBlk()) {
				...
				if (fin != null) {
					...
					if (end == -1) {
						...
						int tableStart = curtable
								.getStartOfHandlerForGuardRegion();
						if (tableStart == start
								&& curtable.getTypeOfHandlerForGuardRegion()
										.equals("FinallyBlock")) {
							fin
									.setEnd(curtable
											.getEndOfHandlerForGuardRegion());
	...
	private boolean skipResetting(ExceptionTable table, MethodInfo minfo) {
		...
		ArrayList neededTables = minfo.getTableListWithGuardsSpecified(start,
				end);
	...
	private ArrayList handleTablesForSynchronizedCode(
			ArrayList allexceptiontables, ArrayList synchronizedTables,
			byte[] info, MethodInfo minfo) {
		...
		for (int s = 0; s < allexceptiontables.size(); s++) {
			...
			if (mexitFound
					&& (jvmInst == JvmOpCodes.MONITORENTER)
					&& (table.getExceptionName() == null || table
							.getExceptionName().equals("<any>"))) {
	...
	private boolean verifyEndOfCatch(TryBlock Try, CatchBlock Catch,
			int gotoOffset, int gotoStart, byte[] codeSrc, MethodInfo minfo) {
		...
		if (jvmInstAtOffset == JvmOpCodes.JSR) // TOOD: handle jsr_w later
		{
			...
			ArrayList list = minfo.getTablesWithGuardRange(Try.getStart(), Try
					.getEnd());
	...
	private int findIfInstructionForThisLoop(byte[] code, int start,
	...
	void populateGotos(byte[] info, MethodInfo minfo) {
		for (int s = 0; s < info.length; s++) {
			if (info[s] == JvmOpCodes.GOTO
					&& isThisInstrStart(minfo.getInststartpos(), s)) // TODO
			// what
			// abt
			// goto_w
			// ????
			{
				...
				gotoStructure.put(new Integer(gotoStart), new Integer(
						gotoAddress));
		...
		// BigInteger.class Method addOne
		{
			if (isThisInstrStart(minfo.getInststartpos(), t)) {
				...
				if (isIf == true) {
					...
					int pos = t + 3;
					if (pos < src.length
							&& isThisInstrStart(minfo.getInststartpos(), pos)) {
					...
					if ((ifjump - 3) >= 0
							&& src[ifjump - 3] == JvmOpCodes.GOTO
							&& isThisInstrStart(minfo.getInststartpos(),
									(ifjump - 3))) {
					...
					if ((ifjump - 3) >= 0
							...
							&& isThisInstrStart(minfo.getInststartpos(),
									(ifjump - 3))) {
						int gj = getJumpAddress(src, (ifjump - 3));
						if (checkForReturn(src, gj, minfo)) {
							return true;
						}
					}

					if (ifjump > e
							&& (src[e] == JvmOpCodes.GOTO || isInstructionIF(src[e]))) {
						return true;
					} else {
						b = false;
					}
	...
	private void mapSwitchStartsWithLabels(int[] offsetValues, int[] labels) {
		...
		for (int s = 0; s < offsetValues.length; s++) {
			...
			int label = labels[s];
			SwitchCaseHelper helper = new SwitchCaseHelper(start, label, false,
					s);
	...
	private int[] getLabelsForOffset(int offset) {
		...
		for (int k = 0; k < caseStartsLabels.size(); k++) {
			SwitchCaseHelper helper = (SwitchCaseHelper) caseStartsLabels
					.get(k);
	...
	// FIXME:
	private int[] getCaseEnd(int desiredPos, int curOffset, int[] offsetValues,
			int[] labels, int def, byte[] info, MethodInfo minfo, Switch switchb) {
		int arr[] = new int[3];
			...
			if (cur == curOffset && (k == desiredPos)) {
				...
				if (k < (offsetValCopy.length - 1)) {
					...
					if (def > cur) {
						...
						int e = findEndOfLastCaseForSwitch(cur, info, minfo,
								switchb);
						arr[0] = cur;
						arr[1] = lbl;
						arr[2] = e;
	...
	// list---loops
	private int checkEndOFLastCatchWRTContainedLoops(CatchBlock Catch,
			ArrayList list) {
		int newend = -1;
		...
		if (possibleCaseEnds.size() > 0) {
			Integer ints[] = (Integer[]) possibleCaseEnds
	...
	private void scanCodeForLocalVariables(MethodInfo minfo) {
		...
		method_name_storeNLoad_Map.put(minfo.getMethodName().concat(
				minfo.getStringifiedParameters()), storeNLoad_Map);
			...
			for (int s = 1; s < code.length; s++) {
				...
				if (isThisInstReturn(t)) {
					java.lang.StringBuffer loadpos = new java.lang.StringBuffer(
							"");
					int loadInstIndex = isPrevInstLoadInst((s - 1), code,
							loadpos, storeNLoad_Map, minfo); // Returns load
					// index if load
					// is present
					// else return
					// -1
					if (loadInstIndex != -1) {
						int pos = (s - 1);
						LocalVariable var = structure.getVariabelAtIndex(
								loadInstIndex, (s - 1));

						if (var == null) // Make another attempt
						{
							var = structure.getVariabelAtIndex(loadInstIndex,
									(s - 2));
							pos = (s - 2);
						}
						if (var == null) // Local variable has to be created
						{
							java.lang.String varName = "Var" + "_" + loadpos
									+ "_" + loadInstIndex;
							java.lang.String rettype = minfo.getReturnType();
							LocalVariable newvar = new LocalVariable(minfo
									.getMethodName().concat(
											minfo.getStringifiedParameters()),
									varName, loadInstIndex);
							newvar.setDataType(rettype);
							newvar.setPrimitive(true);
							newvar.setDeclarationGenerated(false);
							try {
								newvar
										.setLoadORStorePosUsedToCreateVraiable(Integer
												.parseInt(loadpos.toString()
														.trim()));
							} catch (NumberFormatException ne) {
								newvar
										.setLoadORStorePosUsedToCreateVraiable(99999);
							}
			...
			for (int s = 0; s < code.length; s++) {
				...
				if (isThisInstReturn(t)) {
					...
					if (loadInstIndex != -1) {
						...
						LocalVariable var = struc
								.getVariabelAtIndex(loadInstIndex);

						if (var == null) // Local variable has to be created
						{
							java.lang.String varName = "Var" + "_"
									+ loadpos.toString() + "_" + loadInstIndex;
							java.lang.String rettype = minfo.getReturnType();
							LocalVariable newvar = new LocalVariable(minfo
									.getMethodName().concat(
											minfo.getStringifiedParameters()),
				...
				if (isNextInstructionInvokeVirtual(invokeinst)
						|| isNextInstructionInvokeStatic(invokeinst)
						|| isNextInstructionInvokeInterface(invokeinst)
						|| isNextInstructionInvokeSpecial(invokeinst)) {
	...
	private ArrayList VarIndexes(byte[] code, int s, int size) {
		...
		while (count <= size && start >= 0) {
			...
			int loadpos = getLocalVarPosInCodeFromLoadInst(sb, code[start], in,
					start);
			int index = -1;
			if (loadpos != -1) {
	...
	private int isPrevInstLoadInst(int s, byte[] code,
			java.lang.StringBuffer pos, Hashtable ht, MethodInfo minfo) {
		...
		if (isThisInstrStart(minfo.getInststartpos(), s)) {
			...
			case JvmOpCodes.ALOAD_0:
				addCorresPondingStorePosInCode(JvmOpCodes.ASTORE_0, s, code,
						ht, 0, "simple", minfo);
				pos.append(s);
				return 0;

			case JvmOpCodes.ALOAD_1:
				addCorresPondingStorePosInCode(JvmOpCodes.ASTORE_1, s, code,
						ht, 1, "simple", minfo);
				pos.append(s);
				return 1;

			case JvmOpCodes.ALOAD_2:
				addCorresPondingStorePosInCode(JvmOpCodes.ASTORE_2, s, code,
						ht, 2, "simple", minfo);
				pos.append(s);
				return 2;

			case JvmOpCodes.ALOAD_3:
				addCorresPondingStorePosInCode(JvmOpCodes.ASTORE_3, s, code,
						ht, 3, "simple", minfo);
				pos.append(s);
				return 3;

			case JvmOpCodes.DLOAD_0:
				addCorresPondingStorePosInCode(JvmOpCodes.DSTORE_0, s, code,
						ht, 0, "simple", minfo);
				pos.append(s);
				return 0;

			case JvmOpCodes.DLOAD_1:
				addCorresPondingStorePosInCode(JvmOpCodes.DSTORE_1, s, code,
						ht, 1, "simple", minfo);
				pos.append(s);
				return 1;

			case JvmOpCodes.DLOAD_2:
				addCorresPondingStorePosInCode(JvmOpCodes.DSTORE_2, s, code,
						ht, 2, "simple", minfo);
				pos.append(s);
				return 2;
			...
			case JvmOpCodes.DLOAD_3:
				pos.append(s);
				addCorresPondingStorePosInCode(JvmOpCodes.DSTORE_3, s, code,
						ht, 3, "simple", minfo);
				return 3;
			...
			case JvmOpCodes.FLOAD_0:
				addCorresPondingStorePosInCode(JvmOpCodes.FSTORE_0, s, code,
						ht, 0, "simple", minfo);
				pos.append(s);
				return 0;

			case JvmOpCodes.FLOAD_1:
				addCorresPondingStorePosInCode(JvmOpCodes.FSTORE_1, s, code,
						ht, 1, "simple", minfo);
				pos.append(s);
				return 1;

			case JvmOpCodes.FLOAD_2:
				addCorresPondingStorePosInCode(JvmOpCodes.FSTORE_2, s, code,
						ht, 2, "simple", minfo);
				pos.append(s);
				return 2;

			case JvmOpCodes.FLOAD_3:
				addCorresPondingStorePosInCode(JvmOpCodes.FSTORE_3, s, code,
						ht, 3, "simple", minfo);
				pos.append(s);
				return 3;
			...
			case JvmOpCodes.ILOAD_0:
				addCorresPondingStorePosInCode(JvmOpCodes.ISTORE_0, s, code,
						ht, 0, "simple", minfo);
				pos.append(s);
				return 0;

			case JvmOpCodes.ILOAD_1:
				addCorresPondingStorePosInCode(JvmOpCodes.ISTORE_1, s, code,
						ht, 1, "simple", minfo);
				pos.append(s);
				return 1;

			case JvmOpCodes.ILOAD_2:
				addCorresPondingStorePosInCode(JvmOpCodes.ISTORE_2, s, code,
						ht, 2, "simple", minfo);
				pos.append(s);
				return 2;

			case JvmOpCodes.ILOAD_3:
				addCorresPondingStorePosInCode(JvmOpCodes.ISTORE_3, s, code,
						ht, 3, "simple", minfo);
				pos.append(s);
				return 3;
				 ...
				 */
			case JvmOpCodes.LLOAD_0:
				addCorresPondingStorePosInCode(JvmOpCodes.LSTORE_0, s, code,
						ht, 0, "simple", minfo);
				pos.append(s);
				return 0;
			case JvmOpCodes.LLOAD_1:
				addCorresPondingStorePosInCode(JvmOpCodes.LSTORE_1, s, code,
						ht, 1, "simple", minfo);
				pos.append(s);
				return 1;
			case JvmOpCodes.LLOAD_2:
				addCorresPondingStorePosInCode(JvmOpCodes.LSTORE_2, s, code,
						ht, 2, "simple", minfo);
				pos.append(s);
				return 2;
			case JvmOpCodes.LLOAD_3:
				addCorresPondingStorePosInCode(JvmOpCodes.LSTORE_3, s, code,
						ht, 3, "simple", minfo);
				pos.append(s);
				return 3;
		...
		if (p >= 0 && isThisInstrStart(minfo.getInststartpos(), p)) {
			...
			switch (code[p]) {
			case JvmOpCodes.DLOAD:
				addCorresPondingStorePosInCode(JvmOpCodes.DSTORE, p, code, ht,
						code[s], "com", minfo);
				pos.append(p);
				return code[s];
			case JvmOpCodes.LLOAD:
				addCorresPondingStorePosInCode(JvmOpCodes.LSTORE, p, code, ht,
						code[s], "com", minfo);
				pos.append(p);
				return code[s];
			case JvmOpCodes.ILOAD:
				addCorresPondingStorePosInCode(JvmOpCodes.ISTORE, p, code, ht,
						code[s], "com", minfo);
				pos.append(p);
				return code[s];
			case JvmOpCodes.FLOAD:
				addCorresPondingStorePosInCode(JvmOpCodes.FSTORE, p, code, ht,
						code[s], "com", minfo);
				pos.append(p);
				return code[s];

			case JvmOpCodes.ALOAD:
				addCorresPondingStorePosInCode(JvmOpCodes.ASTORE, p, code, ht,
						code[s], "com", minfo);
				pos.append(p);
				return code[s];
	...
	void addCorresPondingStorePosInCode(int lookfor, int loadpos, byte[] code,
			Hashtable map, int matchingindex, java.lang.String type,
			MethodInfo minfo) {
				...
				if (code[s] == lookfor) {
					if (type.indexOf("simple") >= 0) {
						...
						if (curinstindex == matchingindex) {
							int storepos = s;
							map
									.put(new Integer(storepos), new Integer(
											loadpos));
							break;
						}
					}
	...
	private int getLocalVarPosInCodeFromLoadInst(StringBuffer sb, int t,
			StringBuffer in, int where) {
	...
	private void registerStartOfInstuctionsForMethod(byte[] info,
			MethodInfo minfo) {
		int start = 0;
	...
	private boolean isThisInstrStart(ArrayList list, int pos) {
		boolean ok = false;
		if (list == null)
			throw new NullPointerException("START POS IS NULL");
		if (list != null) {
	...
	private void resetEndOfLoops(ArrayList loops, MethodInfo minfo) {
		...
		if (loops != null && unique != null && loops.size() > 0
				&& unique.size() > 0)
			minfo.setLoops(unique);
		int i = 0;
	...
	private ExceptionTable getTableWhoseHandlerIsFinallyForThisTry(
			int trystart, ArrayList trylist) {
		...
		for (int z = 0; z < tables.size(); z++) {
			...
			for (int k = z + 1; k < tables.size(); k++) {
				...
				if (tryblock != tryb) {
					...
					if (temp == trys) {
						ExceptionTable except2 = tryblock
								.getTableUsedToCreateTry();
						sublist.add(except2);
					}
				}
	...
	private void checkForIllegalTriesForList(ArrayList list, ArrayList tries) {
		...
		if (biggest != null) {
			...
			if (guardEnd_biggest < handlerend_next) {
				int handlerstart_biggest = biggest
						.getStartOfHandlerForGuardRegion();
				...
				TryBlock tryblock_biggest = getTryBlockForExceptionTable(
						biggest, tries); // point 3
				if (tryblock_biggest != null) // shud never be null
				{
					FinallyBlock finblk_biggest = tryblock_biggest
							.getFinallyBlock();
					if (finblk_biggest != null) // shud never be null
					{
						TryBlock tryblock_next = getTryBlockForExceptionTable(
								nextTable, tries); // point 3
						...
						{
							FinallyBlock finblk_next = tryblock_next
									.getFinallyBlock();
							if (finblk_next == null) {
	...
	private TryBlock getTryBlockForExceptionTable(ExceptionTable table,
			ArrayList tries) {
	...
	private int findEndOfLastCaseForSwitch(int casestart, byte[] code,
			MethodInfo minfo, Switch switchb) {
		...
		if (casestart > switchb.getDefaultStart()
				&& switchb.getDefaultStart() > 0) {
		...
		if (endOfCase == -1) {
			...
			if (endOfCase > possibleEnd)
				endOfCase = possibleEnd;
			return endOfCase;
		}
	...
	private ArrayList getOuterSwitches(Switch block, ArrayList all) {
		...
		for (int pos = 0; pos < all.size(); pos++) {
			...
			if (blk != block) {
				...
				if (thisStart > blk.getStartOfSwitch()
						&& thisStart < blk.getEndOfSwitch()) {
					list.add(blk);
				}
			}
		}
	...
	private Switch getLastStartSwitchInGroup(ArrayList outerSwitches) {
		if (outerSwitches != null && outerSwitches.size() > 0) {
			...
			if (outerSwitches.size() == 1) {
				return (Switch) outerSwitches.get(0);
			}
	...
	private boolean markLoopAsDoWhileType(int loopStart, byte[] info,
			MethodInfo minfo) {
		int current = loopStart;
		boolean gotoinst = current - 3 >= 0
				&& info[current - 3] == JvmOpCodes.GOTO;
		ArrayList starts = minfo.getInststartpos();
			...
			for (int i = 0; i < attCount; i++) {
				...
				if (name.equals("InnerClasses")) {
					...
					if (methodIndex > 0) {
						...
						clazz.setEnclosingMethodDesc(getUTF8String(nmtype
								.getDescription()));
					}
	...
	private Annotation processAnnotation(DataInputStream dis)
			throws IOException {
		...
		for (int inrctr = 0; inrctr < annotation.getPairCount(); inrctr++) {
			...
			AnnotationElementValue val= processElementValuePair(dis, annotation,
					null, null,annTypeName);
			pair.setValue(val);
	...
	private AnnotationElementValue processElementValuePair(DataInputStream dis,
			Annotation annotation, String type, String arrayTypeName,String valueName)
			throws IOException {
		...
		if (temp.equals("B") || temp.equals("C") || temp.equals("D")
				|| temp.equals("F") || temp.equals("I") || temp.equals("J")
			...
			if (temp.equals("s"))
				elementValue
						.setConstantValueIndexValue(getUTF8String(constValueIndex));
			if (temp.equals("I"))
				elementValue.setConstantValueIndexValue(""
						+ getINTPrimitiveAtCPoolPosition((constValueIndex))
								.getValue());
			if (temp.equals("B"))
				elementValue.setConstantValueIndexValue(""
						+ getINTPrimitiveAtCPoolPosition((constValueIndex))
			...
			if (temp.equals("F"))
				elementValue.setConstantValueIndexValue(""
						+ getFloatPrimitiveAtCPoolPosition((constValueIndex))
								.getValue());
			if (temp.equals("D"))
				elementValue.setConstantValueIndexValue(""
						+ getDoublePrimitiveAtCPoolPosition((constValueIndex))
								.getValue());
			if (temp.equals("L"))
				elementValue.setConstantValueIndexValue(""
						+ getLongPrimitiveAtCPoolPosition((constValueIndex))
								.getValue());
			...
			for (int k = 0; k < numValues; k++) {
				AnnotationElementValue arrayelementValue = processElementValuePair(dis,
						annotation, "array", null,null);
